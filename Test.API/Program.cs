...
app.UseEndpoints(endpoints =>
{
    endpoints.MapControllerRoute(
        name: "default",
        pattern: "{controller=Home}/{action=Index}/{1d?}");

    endpoints.MapControllerRoute(
        name: "tenantRoute",
        pattern: "{tenant}/{controller=Home}/{action=Index}/{1d?}",
        constraints: new { tenant = @"^[a-zA-Zo-9]+$" });
});
...
void RunMigrations()
{
    try
    {
        ...
        var serviceProvider = new ServiceCollection()
            ...
            {
                options.UseSqlServer(connectionString, b => b.MigrationsAssembly("T3st.API"));
            })
